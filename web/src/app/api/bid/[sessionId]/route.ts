...
export async function GET(
  ...
  try {
    ...
    // Verify team admin access or auction-level viewer+ role
    const authResult = await verifyTeamAdminAccess(userId, userEmail, teamId, auctionId)

    if (!authResult.success) {
      // Also allow VIEWER-level participants to read session data
      const supabaseAuth = await createClient()
      const { data: participation } = await supabaseAuth
    ...
    const { data: auction, error: auctionError } = await supabase
      .from('auctions')
      .select('id, name, status, currency_name, currency_icon, budget_per_team, squad_size, queue_state, bidding_type, outcry_config')
      .eq('id', auctionId)
      .maybeSingle()
    ...
    const { data: openRounds } = await supabase
      .from('rounds')
      .select('id, tier_id, player_id, status, closed_at, timer_seconds, opened_at, current_bid_amount, current_bid_team_id, base_price, bid_count')
      .eq('auction_id', auctionId)
      .eq('status', 'OPEN')
    ...
    if (currentRound) {
      ...
      currentRoundData = {
        ...
        // Open outcry round state
        ...(currentRound.current_bid_amount != null ? {
          currentBidAmount: currentRound.current_bid_amount,
          currentBidTeamId: currentRound.current_bid_team_id,
          basePrice: currentRound.base_price,
          bidCount: currentRound.bid_count,
        } : {}),
      }
    }
    ...
    return NextResponse.json({
      auction: {
        ...
        squadSize: squadSize,
        biddingType: (auction as any).bidding_type || 'SEALED_TENDER',
        outcryConfig: (auction as any).bidding_type === 'OPEN_OUTCRY' ? (auction as any).outcry_config : undefined,
      },
      team: {
        id: team.id,
        name: team.name,
