...
export async function POST(
  ...
  try {
    ...
    // Check if auction is OPEN_OUTCRY to set round-level state
    const { data: auction } = await supabase
      .from('auctions')
      .select('bidding_type, outcry_config')
      .eq('id', auctionId)
      .maybeSingle()

    const isOutcry = auction?.bidding_type === 'OPEN_OUTCRY'

    // Get base price from tier if available
    ...
    if (isOutcry && tierId) {
      const { data: tier } = await supabase
        ...
        .eq('id', tierId)
        .maybeSingle()
      basePrice = tier?.base_price ?? null
    }

    // Create a new OPEN round (no timer — auctioneer controls when to close)
    const now = new Date()
    const outcryConfig = auction?.outcry_config as { timer_seconds?: number | null } | null
    const timerSeconds = isOutcry ? (outcryConfig?.timer_seconds ?? null) : null
    ...
    const { data: round, error } = await supabase
      ...
      .select('id, status, player_id, tier_id, opened_at, closed_at, base_price, current_bid_amount, bid_count')
      .single()
