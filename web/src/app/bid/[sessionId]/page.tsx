                          ...
                          <div className="max-w-sm mx-auto">
                            {(auction as any).biddingType === 'OPEN_OUTCRY' && (auction as any).outcryConfig ? (
                              <OpenOutcryBidPanel
                                ...
                                basePrice={(currentRound as any).basePrice ?? basePrice}
                                currentBidAmount={(currentRound as any).currentBidAmount ?? basePrice}
        ...
        {upcomingPlayers && upcomingPlayers.length > 0 && (
          <div className="mt-8">
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center justify-between">
                  <span>Up Next ({upcomingPlayers.length} players)</span>
                  <Button
                    variant="ghost"
                    size="sm"
                    className="h-7 w-7 p-0"
                    onClick={() => setUpNextExpanded(prev => !prev)}
                  >
                    {upNextExpanded ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                  </Button>
                </CardTitle>
              </CardHeader>
              {upNextExpanded && (
