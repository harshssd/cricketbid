...
export interface LiveOutcryBid {
  teamName: string
  amount: number
  sequence: number
  at: string
...
export interface LiveOutcryState {
  currentBid: number
  currentBidTeamName: string | null
  bidCount: number
  timerSecondsLeft: number | null
  recentBids: LiveOutcryBid[]
}
...
export function useLiveAuction(auctionId: string) {
  ...
  const [biddingType, setBiddingType] = useState<'SEALED_TENDER' | 'OPEN_OUTCRY'>('SEALED_TENDER')
  const [outcryState, setOutcryState] = useState<LiveOutcryState>({
    currentBid: 0,
  ...
  const outcryTimerRef = useRef<ReturnType<typeof setInterval> | null>(null)
  const outcryTimerExpiresAtRef = useRef<string | null>(null)
  ...
  const fetchAndHandleState = useCallback(async () => {
    try {
      ...
      if (data.biddingType) setBiddingType(data.biddingType)
      ...
      .on('broadcast', { event: 'outcry-bid' }, ({ payload }) => {
        setOutcryState(prev => {
          ...
          return {
            ...
            currentBidTeamName: payload.teamName,
            bidCount: Math.max(prev.bidCount, payload.sequence),
            timerSecondsLeft: prev.timerSecondsLeft,
            recentBids: [newBid, ...prev.recentBids].slice(0, 30),
          }
        })
        // Update timer
        if (payload.timerExpiresAt) {
          outcryTimerExpiresAtRef.current = payload.timerExpiresAt
        }
      })
      .subscribe()
  ...
  // Outcry timer countdown
  useEffect(() => {
    if (biddingType !== 'OPEN_OUTCRY') return

    const update = () => {
      const expiresAt = outcryTimerExpiresAtRef.current
      if (!expiresAt) {
        setOutcryState(prev => ({ ...prev, timerSecondsLeft: null }))
        return
      }
      const remaining = Math.max(0, Math.floor((new Date(expiresAt).getTime() - Date.now()) / 1000))
      setOutcryState(prev => ({ ...prev, timerSecondsLeft: remaining }))
    }

    update()
    outcryTimerRef.current = setInterval(update, 250)
    return () => {
      if (outcryTimerRef.current) {
        clearInterval(outcryTimerRef.current)
        outcryTimerRef.current = null
      }
    }
  }, [biddingType])

  // Reset outcry state between rounds
  useEffect(() => {
    if (biddingType !== 'OPEN_OUTCRY') return
    // When viewState changes to between_bids or player_up, reset outcry bids
    if (viewState === 'between_bids' || viewState === 'sold_celebration') {
      setOutcryState({
        currentBid: 0,
        currentBidTeamName: null,
        bidCount: 0,
      ...
      outcryTimerExpiresAtRef.current = null
    }
  }, [viewState, biddingType])
